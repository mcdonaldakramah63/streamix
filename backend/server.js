...
// FIX: removed cookie-parser (not installed, not needed — JWT is in Authorization header)
//      profiles + polls routes registered
...
// ── Optional middleware (skip if missing) ─────────────────────────────────────
...
// polls — skip gracefully if not deployed yet
try { app.use('/api/polls', require('./routes/polls')) } catch {
  console.warn('[server] polls route not found — skipping')
}
...
try {
  ...
  console.warn('[server] websocket.js not found')
}

const PORT = process.env.PORT || 5000
server.listen(PORT, () =>
  console.log(`[Streamix] Listening on port ${PORT} [${process.env.NODE_ENV || 'development'}]`)
)
