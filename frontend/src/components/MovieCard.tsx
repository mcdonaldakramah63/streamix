...
export default function MovieCard({ movie, size = 'md' }: Props) {
  ...
  return (
    <Link to={path} className={`card-hover block flex-shrink-0 ${size === 'sm' ? 'w-24 sm:w-28' : 'w-28 sm:w-36'}`}>
      <div className="relative rounded-lg overflow-hidden bg-dark-card group">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-1.5 translate-y-2 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all">
          <div className="text-yellow-400 text-xs font-bold">★ {rating(movie.vote_average)}</div>
        </div>
      </div>
      <div className="mt-1 px-0.5">
        <div className="text-xs font-semibold text-slate-200 truncate leading-tight">{title}</div>
