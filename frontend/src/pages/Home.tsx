...
// Adds: VerticalFeed button, AnimatedPosterCard section, kids mode routing
...
export default function Home() {
  ...
  // Redirect to Kids home if active profile is kids mode
  useEffect(() => {
    if (activeProfile?.isKids) navigate('/kids')
  ...
  // Show TikTok-style feed overlay
  ...
  return (
    <div className="min-h-screen">
      ...
      {heroLoad ? (
        ...
        <div className="relative overflow-hidden" style={{ height:'clamp(280px,55vw,560px)' }}>
          ...
          <div className={`absolute bottom-10 sm:bottom-16 left-4 sm:left-8 right-4 max-w-2xl transition-all duration-500 ${heroVisible?'opacity-100 translate-y-0':'opacity-0 translate-y-2'}`}>
            <div className="flex items-center gap-2 mb-3">
              <span className="badge-brand text-[11px] px-3 py-1 font-bold uppercase tracking-wider">🔥 Trending #{heroIdx+1}</span>
              {hero.vote_average >= 7.5 && <span className="badge-gold text-[11px] px-2.5 py-1">★ {hero.vote_average.toFixed(1)}</span>}
            </div>
            <h1 className="font-bold text-shadow leading-none mb-3"
              style={{ fontFamily:'Syne, sans-serif', fontSize:'clamp(1.6rem,5vw,3.5rem)' }}>
              {hero.title}
            </h1>
            <p className="text-shadow-sm text-slate-300 text-sm leading-relaxed mb-5 line-clamp-2 hidden sm:block max-w-lg">{hero.overview}</p>
            <div className="flex gap-2.5 flex-wrap">
              <button onClick={() => navigate(`/player/movie/${hero.id}`)} className="btn-primary px-5 sm:px-7 py-2.5 sm:py-3 text-sm sm:text-base shadow-brand">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><polygon points="5,3 19,12 5,21"/></svg>
                Play Now
              </button>
              <button onClick={() => navigate(`/movie/${hero.id}`)} className="btn-secondary px-5 sm:px-7 py-2.5 sm:py-3 text-sm sm:text-base">More Info</button>
              {/* TikTok feed button */}
              <button onClick={() => setShowFeed(true)}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl glass border border-white/20 text-white text-sm font-medium hover:border-brand/50 transition-all active:scale-95">
      ...
      {/* ── For You Feed promo strip ── */}
      <div className="mx-3 sm:mx-6 mb-6">
        <button
          onClick={() => setShowFeed(true)}
          className="w-full rounded-2xl p-4 flex items-center gap-4 transition-all active:scale-[0.99] hover:border-brand/30"
          style={{ background:'linear-gradient(135deg, rgba(20,184,166,0.08) 0%, rgba(20,184,166,0.03) 100%)', border:'1px solid rgba(20,184,166,0.2)' }}>
          <div className="w-12 h-12 rounded-2xl bg-brand/15 flex items-center justify-center flex-shrink-0">
      ...
      <section className="px-3 sm:px-6 pb-12 mt-2">
        ...
        {gridLoad ? (
          ...
          <>
            ...
            {!hasMore && popular.length>0 && <p className="text-center text-xs text-slate-700 py-8">— End of list —</p>}
          </>
        )}
      </section>
    </div>
