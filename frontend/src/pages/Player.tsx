...
export default function Player() {
  ...
  const doSave = useCallback((progress: number) => {
    ...
    if (!m || !id) return
    const durationMins = type === 'tv' ? (ep?.runtime || TYPICAL_EP) : (m.runtime || TYPICAL_MOVIE)
    save({
      ...
      episodeName: ep?.name,
      progress, timestamp: curTime.current||0, duration: curDur.current||undefined, durationMins,
    })
      ...
      return (
        <div className="aspect-video flex flex-col items-center justify-center bg-dark-surface gap-3">
          ...
          <p className="text-sm text-slate-500">
            {animeLoading ? 'Finding stream…' : 'Loading…'}
          </p>
        </div>
      )
    }
        ...
        <div className="flex items-center gap-3 px-3 sm:px-4 py-3 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {type === 'tv' && (
                <p className="text-brand text-xs">S{season} · E{episode}{curEp ? ` — ${curEp.name}` : ''}</p>
              )}
            ...
            {type === 'tv' && (
              <div className="flex items-center gap-2 px-3 sm:px-0 mt-3">
                <button onClick={() => prevEp && goEpisode(prevEp.episode_number)} disabled={!prevEp}
                  className="flex-1 btn-secondary py-2.5 text-sm disabled:opacity-30 flex items-center justify-center gap-2">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="m15 18-6-6 6-6"/></svg>
                  Previous
                </button>
                <span className="text-xs text-slate-700 tabular-nums px-1">{episode}/{episodes.length}</span>
                    ...
                    {(media as any)?.seasons?.filter((s: any) => s.season_number > 0).map((s: any) => (
                      <button key={s.id} onClick={() => goEpisode(1, s.season_number)}
                        className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${
                          season === s.season_number ? 'bg-brand text-dark' : 'bg-dark-card border border-dark-border text-slate-400 hover:text-white'
                        }`}>
                        S{s.season_number}
                      </button>
                    ))}
                ...
                <div className="overflow-y-auto" style={{ maxHeight: '60vh' }}>
                  {episodes.length === 0 ? (
                    ...
                    <button key={ep.id} onClick={() => goEpisode(ep.episode_number)}
                      className={`w-full flex items-center gap-3 p-3 text-left border-b border-dark-border/40 transition-all hover:bg-dark-hover ${
                        ep.episode_number === episode ? 'bg-brand/8 border-l-2 border-l-brand' : ''
                      }`}>
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black flex-shrink-0 transition-colors ${
                        ep.episode_number === episode ? 'bg-brand text-dark' : 'bg-dark-surface text-slate-500'
                      }`}>
                        {ep.episode_number}
                      </div>
                      <div className="flex-1 min-w-0">
                      ...
                      {ep.episode_number === episode && (
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" className="text-brand flex-shrink-0">
